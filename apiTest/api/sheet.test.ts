  ...
  describe('cell manipulations', () => {
    ...
    describe('query', () => {
      it('should find rows by ID', async () => {
        // A: ROW, B: ID
        const actual = await api.query(
          'select A where B = "storage-a"',
          'storages'
        )
  ...
  describe('sheet manipulations', () => {
    ...
    afterEach(async () => {
      ...
      if (meta.data.sheets.length > 2) {
        const targets = meta.data.sheets.filter(
          (s) =>
            s.properties.title != 'belongings' &&
            s.properties.title != 'storages'
        )
